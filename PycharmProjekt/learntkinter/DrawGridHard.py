...
import sudokugeneratorHard as sgH
import correctingHard as corH

entries = {}


def drawGridHard(entries):
    root = tk.Tk()
    root.title("Sudoku")
    tableheight = 9
    tablewidth = 9
    ...
    submitButton = tk.Button(root, text="Correcting", command = lambda: corH.getEingabenHard(entries))
    submitButton.grid(row=10, column=10)
    sgH.createHard(entries)
