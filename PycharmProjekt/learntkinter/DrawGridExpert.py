...
import sudokugeneratorExpert as sgX
import correctingExpert as corX
entries = {}


def drawGridExpert(entries):
    root = tk.Tk()
    root.title("Sudoku")
    tableheight = 9
    tablewidth = 9
    ...
    submitButton = tk.Button(root, text="Correcting", command = lambda: corX.getEingabenExpert(entries))
    submitButton.grid(row=10, column=10)
    sgX.createExpert(entries)
