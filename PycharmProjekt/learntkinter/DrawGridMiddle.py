...
import sudokugeneratorMiddle as sgM
import correctingMiddle as corM
entries = {}


def drawGridMiddle(entries):
    root = tk.Tk()
    root.title("Sudoku")
    tableheight = 9
    tablewidth = 9
    ...
    submitButton = tk.Button(root, text="Correcting", command = lambda: corM.getEingabenMiddle(entries))
    submitButton.grid(row=10, column=10)
    sgM.createMiddle(entries)